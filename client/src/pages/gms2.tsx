...
// Enhanced function to detect and apply shortcodes for classification, address, and caller info
  const detectAndApplyShortcodes = async (text: string) => {
    ...
    // Address shortcode: =[stad]/[straatnaam] [huisnummer] OR =[stad]/[straatnaam]
    if (lastLine.startsWith('=')) {
      const cleanInput = lastLine.substring(1).trim();
      ...
      if (cleanInput.includes('/')) {
        ...
        if (parts.length >= 2) {
          ...
          if (stad && straatnaam) {
            console.log(`📍 Address shortcode detected: ${stad} / ${straatnaam}${huisnummer ? ' ' + huisnummer : ''}`);
