...
const ROTTERDAM_AREAS: Record<string, [number, number][]> = {
  ...
  // Schiedam: Correcte gemeentegrens, tussen Vlaardingen en Rotterdam
  'Schiedam': [
    [51.9050, 4.3700],
    [51.9150, 4.3750],
    [51.9200, 4.3850],
  ...
  // Vlaardingen: Correcte gemeentegrens, westelijk van Schiedam
  'Vlaardingen': [
    [51.9000, 4.3200],
    [51.9100, 4.3250],
    [51.9150, 4.3350],
...
function EditBasisteamForm({ 
  ...
  const generatePolygonFromWijken = () => {
    ...
    let selectedPolygon: [number, number][] = [];

    // Find the first matching wijk polygon
    for (const wijk of wijken) {
      if (ROTTERDAM_AREAS[wijk]) {
        selectedPolygon = ROTTERDAM_AREAS[wijk] as [number, number][];
        break;
      }
    }
